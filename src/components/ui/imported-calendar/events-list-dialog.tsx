...
import {
  ...
  ModalTitle,
  ModalTrigger,
} from "@/components/ui/imported-calendar/responsive-modal";
...
import { dayCellVariants } from "@/components/ui/imported-calendar/day-cell";
  ...
  const defaultTrigger = (
    <span className="cursor-pointer">
      <span className="sm:hidden">+{hiddenEventsCount}</span>
        ...
        {hiddenEventsCount}
        <span className="mx-1">{messages.moreEventsSuffix}</span>
      </span>
    </span>
  );
          ...
          <ModalTitle className="my-2">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-medium">
                {messages.eventsOnDate}{" "}
                {format(date, "EEEE, d 'de' MMMM yyyy", { locale: dateLocale })}
              </p>
            </div>
          </ModalTitle>
        </ModalHeader>
