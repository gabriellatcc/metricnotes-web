...
import { DraggableEvent } from "@/components/ui/imported-calendar/draggable-event";
import { ResizableEvent } from "@/components/ui/imported-calendar/resizable-event";
...
const calendarWeekEventCardVariants = cva(
  "flex select-none flex-col gap-0.5 truncate whitespace-nowrap rounded-md border px-2 py-1.5 text-xs focus-visible:outline-offset-2",
  {
    variants: {
      color: {
        // Colored variants
        blue: "border-(--accent-foreground)00 bg-(--accent-foreground)/50 text-(--primary) hover:bg-(--accent-foreground) dark:border-(--primary) dark:bg-(--accent-foreground)/50 dark:text-(--accent-foreground) dark:hover:bg-(--accent-foreground)",
        green:
        ...
        // Dot variants
        "blue-dot":
          "border-border bg-card text-foreground hover:bg-accent [&_svg]:fill-(--primary) dark:[&_svg]:fill-(--primary)00",
        "green-dot":
...
export function EventBlock({ event, className }: IProps) {
  const { badgeVariant, use24HourFormat, dateLocale } = useCalendar();
        ...
        <EventDetailsDialog event={event}>
          ...
          >
            ...
            {durationInMinutes > 25 && (
              <p>
                {formatTime(start, use24HourFormat, dateLocale)} -{" "}
                {formatTime(end, use24HourFormat, dateLocale)}
              </p>
            )}
          </button>
        </EventDetailsDialog>
